...
export class TypedEventEmitter<
    TEventMap extends Record<string, Record<string, unknown>>,
> {
    ...
    on<E extends SpecificEventName<TEventMap>>(
        ...
        if (!eventName.includes(":")) {
            throw new Error(
                "Expected specific event name with format 'domain:event'",
            );
        }
    ...
    subscribe<E extends SpecificEventName<TEventMap>>(
        ...
        let resolvers: Array<
            (value: IteratorResult<ExtractPayload<TEventMap, E>>) => void
        > = [];
        let isDone = false;
        ...
        return {
            ...
            async next(): Promise<
                IteratorResult<ExtractPayload<TEventMap, E>, undefined>
            > {
                if (isDone) {
    ...
    subscribeDomain<D extends DomainEventName<TEventMap>>(
        ...
        let resolvers: Array<
            (value: IteratorResult<DomainEvents<TEventMap, D>>) => void
        > = [];
        let isDone = false;
        ...
        return {
            ...
            async next(): Promise<
                IteratorResult<DomainEvents<TEventMap, D>, undefined>
            > {
                if (isDone) {
                    return { value: undefined, done: true };
                }
            ...
            async throw(
                error: unknown,
            ): Promise<IteratorResult<DomainEvents<TEventMap, D>, undefined>> {
                isDone = true;
                unsubscribe();
    ...
    emit<E extends SpecificEventName<TEventMap>>(
        ...
        if (parts.length >= 2) {
            ...
            const domainSet = this.domainHandlers.get(
                domain as DomainEventName<TEventMap>,
            );
            if (domainSet) {
