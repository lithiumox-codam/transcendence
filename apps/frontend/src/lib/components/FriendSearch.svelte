...
<div class="relative w-full max-w-md">
    <div class="relative">
        <input
            ...
            autocomplete="off"
            class="w-full px-4 py-2 bg-gray-700/20 border border-gray-600/20 rounded-md text-gray-200 placeholder-gray-400/40 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
        />
        {#if searchTerm}
            <button
                class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400/70 hover:text-white/70 text-xl w-6 h-6 flex items-center justify-center rounded-full hover:bg-gray-600/30 transition-colors"
                onclick={() => {
                    searchTerm = "";
                }}
    ...
    {#if results.length > 0}
        <div
            class="absolute z-50 w-full mt-1 bg-gray-800/80 border border-gray-700/20 rounded-md shadow-lg max-h-80 overflow-y-auto"
        >
