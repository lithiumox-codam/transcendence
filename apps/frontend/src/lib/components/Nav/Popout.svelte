...
{#if popout.shown && popout.component}
	<div
		class="fixed top-0 left-0 w-screen h-screen z-40 bg-black/50"
		transition:fade={{ duration: 200 }}
		role="presentation"
	>
		<div
			...
			style="position: absolute; top: {popout.position.y}px; left: {popout
				.position.x}px; width: {popout.size.width}px; height: {popout
				.size.height}px;"
			class="bg-black text-white shadow-lg rounded-lg overflow-hidden border border-gray-800 flex flex-col"
		>
			<!-- Header with drag functionality -->
			<header
				class="flex items-center justify-between p-2 bg-gray-950 border-b border-gray-800 cursor-move"
				role="toolbar"
				aria-label="Popout window controls"
			>
				<button
